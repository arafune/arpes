...
from src.arpes.workflow import (
    DataProvider,
    consume_data,
    go_to_cwd,
    go_to_figures,
...
@pytest.mark.skip
@patch("src.arpes.workflow._open_path")
def test_go_to_figures(mock_open_path: MagicMock) -> None:
    """Test that go_to_figures opens the correct path."""
    with patch("src.arpes.workflow.path_for_plot", return_value="figures"):
        go_to_figures()
...
def test_go_to_workspace(mock_open_path: MagicMock) -> None:
    ...
    with patch("src.arpes.workflow.CONFIG", {"WORKSPACE": {"path": "workspace_path"}}):
        go_to_workspace()
...
@patch("src.arpes.workflow.DataProvider.publish")
...
@patch("src.arpes.workflow.DataProvider.consume")
...
@patch("src.arpes.workflow.DataProvider.summarize_clients")
...
def test_data_provider_init() -> None:
    ...
    with patch("src.arpes.workflow.Path.mkdir") as mock_mkdir:
...
@pytest.mark.skip
@patch("src.arpes.workflow.dill.load")
@patch("src.arpes.workflow.Path.open")
...
@pytest.mark.skip
@patch("src.arpes.workflow.dill.dump")
@patch("src.arpes.workflow.Path.open")
