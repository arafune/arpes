...
@pytest.fixture
def scan_desc_with_path(tmp_path):
...
@pytest.fixture
def scan_desc_with_file(tmp_path):
    file_path = tmp_path / "datafile2.dat"
    file_path.write_text("dummy data")
    return {"file": str(file_path)}


def test_resolve_frame_locations_with_existing_path(scan_desc_with_path):
...
def test_resolve_frame_locations_with_existing_file(scan_desc_with_file):
    endstation = DummyEndstation()
...
def test_resolve_frame_locations_file_not_found(monkeypatch):
    ...
    monkeypatch.setattr("arpes.endstations.single_file_endstation.get_data_path", lambda: None)
    with pytest.raises(RuntimeError):
        endstation.resolve_frame_locations(scan_desc)


def test_resolve_frame_locations_none():
    endstation = DummyEndstation()
